...
with rasterio.open('data/raw/dem_lidar/dem_clipped.tif') as src:
    dem_array = src.read(1).astype(float)
    profile   = src.profile
    transform = src.transform
    res_x     = abs(transform.a)   # pixel width in meters
    res_y     = abs(transform.e)   # pixel height in meters
    bounds    = src.bounds

print(f"  Pixel resolution: {res_x:.1f} x {res_y:.1f} m")

dem_array[dem_array < -9000] = np.nan

# ── 2. Compute terrain attributes with correct geotransform ──────────────────
print("Computing terrain attributes...")
...
# ── 3. Write corrected rasters to temp files ──────────────────────────────────
...
# ── 4. Zonal stats per segment (500m buffer) ──────────────────────────────────
...
# Stream proximity: low elevation areas within buffer = drainage
# We approximate as % of pixels below mean elevation (simple proxy)
...
with rasterio.open(dem_path)   as d_src, \
    ...
    for idx, row in segs_buf.iterrows():
        ...
        # Stream proximity proxy: % pixels in lowest 10th percentile of elevation
        try:
...
df.to_csv('data/processed/terrain_features/terrain_features.csv', index=False)

print(f"\nTerrain features saved: data/processed/terrain_features/terrain_features.csv")
print(f"Shape: {df.shape}")
print(f"\nSlope stats (should be 0-60 range for mountain terrain):")
print(f"  Mean slope: {df.mean_slope_deg.mean():.1f}°")
print(f"  Max slope:  {df.max_slope_deg.max():.1f}°")
print(f"\nSample output:")
print(df.head(5).to_string(index=False))
print("\nPhase 3 (fixed) complete.")
